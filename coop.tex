...
\end{lstlisting}
  ...
  Thus the only safe rule for double pointers is an \textit{\invariant{}} one.
  \begin{mathpar}
...
\begin{example}[Dynamic types are screwy]
  ...
  Consider a language that has a \texttt{MyType} to represent the \dyanamic{} type of a term.
  We may define:
\begin{lstlisting}[language=Java]
class Point {
    int x,y;
...
\end{enumerate}
This is different in each language. In Java for step 1 any method in the \static{} class and any of its superclass's are equally valid options. In C++ and C\# it starts at the \static{} type of the \reciever{} and goes up the class hierarchy until a suitable method is found.
...
\end{lstlisting}
  ...
  Because the \dyanamic{} types of $c_1$ and $c_2$ could be some subtype of $I$.
...
\end{lstlisting}
  ...
  \begin{enumerate}
  \item In line 19 of \texttt{clientTest} \texttt{Client}'s constructor is invoked.
