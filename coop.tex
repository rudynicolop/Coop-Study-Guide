...
\begin{itemize}
\item A superclass shoud not change calls to \dynamic{}ally bound methods.
...
Some (experimental) languages support \textit{multipe dispatch}.
\begin{lstlisting}[language=Java]
class Animal {
    boolean equals (Animal a) {
        // General code for Animals
...
The dynamic dispatch order is determined by \textit{linerization}.
...
\begin{definition}[Java Virutal Machine]
...
\item A heap where objects are stored in memory. The stack and regiters may have pointers to the heap.
\end{itemize}
\end{definition}
...
\begin{equation}
\wkin{p} = \bigsqcup \{ \wkout{q} \mid q ~ \text{predeces} ~ p \}
\end{equation}

We want to take all of the precedessor instructions of $p$, take the joins of their types in their stacks and registers.
...
Here we are going to try to demonstrate that Java forces one to twist themselves into a prezle if they just want to use standard type parameters.
...
However it does not know that the paramater type and return type are the same.
\begin{lstlisting}[language=Java]
...
If one chooses to instantiate \texttt{Erausre} with the same types any call to foo will be ambiguous and cause a typechecking error.
\begin{lstlisting}[language={[Sharp]C}]
...
\epigraph{\itshape The big lie of object-oriented programming is that
objects provide encapsulation.}{John Hogg.}
