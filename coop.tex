...
The type rule for array assignment forces right hand side to be committed.
Note that we do not allow overloads from differences in construction types.
Checkout spec sharp.
Sometimes better to call super constructor at end of sub constructor.

\subsection{Global Initialization}

If a module A uses a global variable from module B, main needs to init B first.
In C++ the programmer needs to declare modules in right order.
In Java static initializers calling static stuff from other class causes problems.
